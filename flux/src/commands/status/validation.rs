...
pub fn validate_markdown_files(
    dir: &std::path::Path,
    entity_type: &str
) -> Result<usize> {
    let mut issues = 0;
...
fn validate_runner_track_refs(
    ...
    for entry in fs::read_dir(&runners_dir)? {
        ...
        if path.is_file() && path.extension().is_some_and(|e| e == "md") {
            if let Ok(content) = fs::read_to_string(&path) {
                if let Ok(doc) = MarkdownDocument::parse(&content) {
                    if let Some(assigned_track) = doc.get_frontmatter(
                        "assigned_track"
                    ) {
...
fn validate_signal_runner_refs(
    work_dir: &WorkDir,
    ...
    for entry in fs::read_dir(&signals_dir)? {
        ...
        if path.is_file() && path.extension().is_some_and(|e| e == "md") {
            if let Ok(content) = fs::read_to_string(&path) {
                if let Ok(doc) = MarkdownDocument::parse(&content) {
                    if let Some(target_runner) = doc.get_frontmatter(
                        "target_runner"
                    ) {
