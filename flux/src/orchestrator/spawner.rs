...
/// Returns a Session object with tmux_session and pid populated.
...
/// List all flux-related tmux sessions
///
...
mod tests {
    ...
    fn test_generate_session_name() {
        let name = generate_session_name("stage-1", "flux");
        assert_eq!(name, "flux-stage-1");
    ...
    fn test_tmux_session_info_creation() {
        let info = TmuxSessionInfo {
            name: "flux-stage-1".to_string(),
            created: DateTime::from_timestamp(1704067200, 0),
            attached: true,
            windows: 2,
        };

        assert_eq!(info.name, "flux-stage-1");
