...
/// List flux branches (branches starting with flux/)
pub fn list_flux_branches(repo_root: &Path) -> Result<Vec<String>> {
    let output = Command::new("git")
        .args(["branch", "--list", "flux/*"])
        .current_dir(repo_root)
        .output()
        .with_context(|| "Failed to list flux branches")?;
...
/// Clean up flux branches that have been merged
pub fn cleanup_merged_branches(target_branch: &str, repo_root: &Path) -> Result<Vec<String>> {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    let merged_flux_branches: Vec<String> = stdout
        .lines()
        .map(|s| s.trim().trim_start_matches('*').trim().to_string())
        .filter(|s| s.starts_with("flux/"))
        .collect();

    let mut deleted = Vec::new();
    for branch in merged_flux_branches {
...
/// Get the stage ID from a flux branch name
pub fn stage_id_from_branch(branch_name: &str) -> Option<String> {
    branch_name.strip_prefix("flux/").map(String::from)
}

/// Generate flux branch name from stage ID
...
mod tests {
  ...
  flux/stage-1 def5678 Add feature
...
"#;
        ...
        assert_eq!(branches[1].name, "flux/stage-1");
    ...
    fn test_stage_id_from_branch() {
        assert_eq!(
            stage_id_from_branch("flux/stage-1"),
    ...
    fn test_branch_name_for_stage() {
        assert_eq!(branch_name_for_stage("stage-1"), "flux/stage-1");
        assert_eq!(branch_name_for_stage("my-feature"), "flux/my-feature");
    }
}
