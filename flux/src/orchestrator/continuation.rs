...
//!
//! ```ignore
//! use flux::orchestrator::continuation::{prepare_continuation, ContinuationConfig};
//! use std::path::Path;
//!
...
pub fn continue_session(
    ...
    if config.auto_spawn {
        let spawned_session = spawn_session(stage, worktree, &config.spawner_config)
            .context("Failed to spawn tmux session for continuation")?;

        // Update session with tmux details
        if let Some(tmux_name) = spawned_session.tmux_session {
            session.set_tmux_session(tmux_name);
        }
        if let Some(pid) = spawned_session.pid {
            session.set_pid(pid);
        }
...
fn load_stage(work_dir: &Path, stage_id: &str) -> Result<Stage> {
    ...
    if !stage_path.exists() {
        bail!(
            "Stage file not found: {}. Run 'flux stage create' first.",
            stage_path.display()
        );
    }
...
fn load_worktree_path(work_dir: &Path, worktree_id: &str) -> Result<PathBuf> {
    ...
    if !worktree_path.exists() {
        bail!(
            "Worktree directory does not exist: {}. Create it with 'flux worktree create'.",
...
"#
    ...
    fn test_continuation_config_default() {
        ...
        assert_eq!(config.spawner_config.tmux_prefix, "flux");
    ...
    fn test_prepare_continuation_with_handoff() {
        ...
        assert_eq!(context.branch, format!("flux/{stage_id}"));
    }
...
"#;
    ...
    fn test_session_to_markdown() {
        ...
        assert!(markdown.contains("**Tmux**: flux-stage-1"));
    }
}
