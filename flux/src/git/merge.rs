...
/// 2. Merge stage branch (flux/{stage_id})
...
pub fn conflict_resolution_instructions(
    ...
    instructions.push_str(&format!(
        "Merge conflict detected when merging flux/{stage_id} into {target_branch}\n\n"
    ));
    ...
    instructions.push_str(&format!("  3. git merge flux/{stage_id}\n"));
    ...
    instructions.push_str(&format!(
        "  7. flux merge {stage_id} (to clean up worktree)\n"
    ));
...
mod tests {
    ...
    fn test_conflict_resolution_instructions() {
        ...
        assert!(instructions.contains("flux/stage-1"));
